...
def create_recording():
    ...
    try:
        ...
        app.logger.info(f"Authenticated user: {g.user}")
        # ここに録音データの処理を追加
        # リクエストから音声ファイルを取得
        ...
        date_recorded = datetime.strptime(date_recorded, '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%Y-%m-%d %H:%M:%S')
        ...
        return jsonify({"message": "Recording created successfully", "data": recording_data}), 201
